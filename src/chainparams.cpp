...
{    ///
    const char* pszTimestamp = "We will destroy the business world by creating a new one!!!! 02/19/2018";       ////
...
public:
    CMainParams() {
        ...
        consensus.powLimit = uint256S("7ffffffffffffffffffffffffffffffffffffffffffffffffffffffff");            ////
...
public:
    CRegTestParams() {
        ...
        genesis = CreateGenesisBlock(1496688602, 2, 0x207fffff, 1, 8 * COIN);                 //// change var 1 5 only
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x"));                  ////
        assert(genesis.hashMerkleRoot == uint256S("0x"));                      ////
